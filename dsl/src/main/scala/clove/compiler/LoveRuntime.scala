...
object LoveRuntime:
  ...
  def wrap(world: World): String =
    ...
    val cameraSetup = world.camera match
      case Some(CameraConfig(follow, threshold, axis)) =>
        s"""local camera = {
           |  x = 0,
           |  y = 0,
           |  follow = "${follow.name}",
...
local function updateCamera()
  local e = entities[camera.follow]
  if not e then return end
  if camera.axis == "Horizontal" or camera.axis == "Both" then
    if e.x > camera.threshold then
      camera.x = e.x - camera.threshold
    end
  end
  if camera.axis == "Vertical" or camera.axis == "Both" then
    if e.y > camera.threshold then
      camera.y = e.y - camera.threshold
    end
  end
end
