...
def entityTest(args: String*): Unit =
  ...
  val setup = world {
    platform("ground", 0, 450, 2400, 40)

    for i <- 0 until nEntities do
      val e = entity(s"test_$i")
        .onSpawn {
          setState("x", (50.0 + (i % 40) * 10.0))
          setState("y", (10.0 + (i / 40) * 10.0))
          setSize(8.0, 8.0)
          setState("goRight", true)
        }
        .onUpdate {
          perform(Effect.Gravity())
          when(getState("x") <= 50.0)  { setState("goRight", true)  }
          when(getState("x") >= 150.0) { setState("goRight", false) }
          when(getState("goRight")) { move(50.0, 0.0) } otherwise { move(-50.0, 0.0) }
        }
      spawn(e)
  ...
  println(s"Generated test: $nEntities entities, $nRegions regions")